...
ROWS = [
    ("host",),
    ("kv", "Now", "BSCS Student (Rising 5th Sem) @ UCP Lahore"),
    ("kv", "Role", "OOP Lab Class Representative @ UCP"),
    ("kv", "Focus", "Backend Development & Core Programming"),
    ("kv", "Edu", "BS Computer Science (3.68 CGPA)"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "C++, C#, SQL, x86 Assembly"),
    ("kv", "Backend", ".NET, Node.js"),
    ("kv", "Tools", "PowerShell, Docker, OBS Studio, FFmpeg"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Winner: 'Container Commander' @ IEEE Think2Code"),
    ("bul", "Active participant @ IEEE UCP Techathon"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">MCP2YT@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">MCP2YT</tspan><tspan fill="{MUTED}">@</tspan>'
